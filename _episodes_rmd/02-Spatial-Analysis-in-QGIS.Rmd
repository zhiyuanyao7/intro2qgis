...
Projecting a map into different projections means drawing the earth surface onto a âa flat piece of paperâ. There are a variety of methods to âdrawâ, among which there are three major methods: planar (left), conic (middle), and cylindrical (right) projections. 
...
The Apartments layer we mapped into QGIS canvas are locations represented by X and Y Coordinates (latitude and longitude on the Earth surface). To analyze the layer, we need to project the Apartments layer from latitude and longitude (unit in degrees) to a âflat paperâ using a suitable method. We will project the Apartments layer into **NAD83(2011)/California Zone 5**. 
...
1. Looking at the renamed layer: Apartments_sp, you can find there is a button (looks like a caterpillar). This button indicates the layer is a temporal layer and is not saved. To save this layer, click the âcaterpillarâ button, a Save Scratch Layer window pops up. 
...
3. After saving the layer, go back and check the Apartments_sp in the Layer panel. The âcaterpillarâ button disappears, indicating the layer is saved. 
...
3. join the bufferâs attribute to apartments layer
...
We counted the number of grocery stories with each buffer, and the information is stored in GroceryCount Layer. Letâs check the attribute table, find the information (NUMPOINTS) and join the attribute table to Apartments_sp. 
...
3. To join GroceryCountâs attribute table to Apartments_sp, we need to 
...
8. here is a âcaterpillarâ button, indicating this is a scratch layer that is not saved on local drive. To save it, click the âcaterpillarâ button and save it as Apartment_GroceryCount.shp. 
...
4. Click the functions from Fields and Values and Operates in the middle panel and make the expressions: âNUMPOINTSâ >= AND âPriceâ <= 1300. Then click Selected Features. 
